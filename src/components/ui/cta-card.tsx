...
const CtaCard = React.forwardRef<HTMLDivElement, CtaCardProps>(
  ({ className, imageSrc, imageAlt, title, subtitle, description, buttonText, onButtonClick, ...props }, ref) => {
    return (
      ...
      >
        {/* Image Section */}
        <div className="md:w-1/3 w-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
          <img
            src={imageSrc}
            alt={imageAlt}
            loading="lazy"
        ...
        <div className="md:w-2/3 w-full p-6 md:p-8 flex flex-col justify-center">
          <div>
            ...
            <div className="mt-6">
              <Button size="lg" onClick={onButtonClick} className="bg-gradient-to-r from-green-500 to-green-600 text-white">
                {buttonText}
              </Button>
            </div>
          </div>
