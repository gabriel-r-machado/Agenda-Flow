...
const LEGEND_ITEMS: LegendItem[] = [
  { name: 'Threat Intel', color: '#FAE5F6' }, // Lightest Pink
  { name: 'DLP', color: '#EE4094' },          // Medium Pink
  { name: 'SysLog', color: '#BB015A' },       // Darkest Pink
];
...
// Data for StackedNormalizedAreaChart. Order matches LEGEND_ITEMS and CHART_COLOR_SCHEME.
// First series (Threat Intel) will be at the bottom of the stack.
...
const METRICS_DATA: MetricInfo[] = [
  {
    ...
    trendBaseColor: '#E84045', // Red for upward (worse) trend
  ...
  },
  {
    ...
    value: '10%',
    TrendIcon: TrendDownIcon, // Green for downward (better) trend for escalation rate
    trendBaseColor: '#40E5D1',
    trendStrokeColor: '#40E5D1',
    delay: 0.1,
    iconFillColor: '#E84045', // Main icon still red as it's an alert/metric
  },
];

const DetailedNormalizedIncidentReport: React.FC = () => {
  return (
    <>
      <style jsx global>{`
        :root {
      ...
      <div className="flex flex-col pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-md min-h-[580px] overflow-hidden transition-colors duration-300">
        ...
        {/* Chart */}
        <div className="reaviz-chart-container h-[200px] px-2"> {/* Added padding for labels */}
          <StackedNormalizedAreaChart
              ...
              <LinearXAxis
                ...
                tickSeries={
                  <LinearXAxisTickSeries
                    ...
                    tickSize={10} // Optimized tick size
                  />
                }
              />
            }
            yAxis={
              <LinearYAxis
                axisLine={null}
                tickSeries={<LinearYAxisTickSeries line={null} label={null} tickSize={10} />} // Optimized tick size
              />
            }
            series={
              <StackedNormalizedAreaSeries
